...
class ProteinNode:
    def __init__(self,ProteinID,ProteinLen,PrimarySeq,SecondarySeq,PHI,PSI,ACC):
...
def load_FastaFile(FastaFile,IsESM=True):
    f=open(FastaFile,'r')
    if IsESM:
    ...
    while True:       
        ...
        for i in range(ProteinLen):
            if ACC[i]!='X':
                tmp_list.append(float(ACC[i]))
            else:
                tmp_list.append(float('nan'))
        ACC=torch.tensor(tmp_list,dtype=torch.float)                
        Node=ProteinNode(ProteinID,ProteinLen,PrimarySeq,SecondarySeq,PHI/180.0,PSI/180.0,ACC)
        Data.append(Node)
    f.close()
    return Data


def load_FastaFile2(FastaFile,IsESM=True):
    f=open(FastaFile,'r')
    if IsESM:
        AA_Dict={ 'L':4, 'A':5, 'G':6, 'V':7, 'S':8, 'E':9, 'R':10, 'T':11, 'I':12, 'D':13, 'P':14, 'K':15, 'Q':16, 'N':17, 'F':18, 'Y':19, 'M':20, 'H':21, 'W':22,
                  'C':23, 'X':24, 'B':25, 'U':26, 'Z':27, 'O':28}
    ...
    while True:       
        ...
        Node=ProteinNode(ProteinID,ProteinLen,PrimarySeq,None,PHI/180.0,PSI/180.0,None)
        Data.append(Node)
    f.close()
    return Data
...
def load_four_testsets(IsESM=True):
   Test2020_HQ_list=load_FastaFile2('data/data/TEST2020_HQ.txt',IsESM)
   Test2020_HQ_list.sort(key=lambda Node: Node.ProteinLen,reverse=False)   
   CASP12_list=load_FastaFile2('data/data/CASP12.txt',IsESM)
   CASP12_list.sort(key=lambda Node: Node.ProteinLen,reverse=False) 
   CASP13_list=load_FastaFile2('data/data/CASP13.txt',IsESM)
   CASP13_list.sort(key=lambda Node: Node.ProteinLen,reverse=False) 
   CASPFM_list=load_FastaFile2('data/data/CASPFM.txt',IsESM)
